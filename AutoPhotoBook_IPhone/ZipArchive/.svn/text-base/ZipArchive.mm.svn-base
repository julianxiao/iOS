...
- (void) updateProgress: (NSNumber *) value{
	[[GlobalController instance] updateDownloadProgress: [value intValue] * 100 /TOTAL_ARCHIVED_FILE_COUNT];
	if ([value intValue] == TOTAL_ARCHIVED_FILE_COUNT) {
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	do{
		...
		unzCloseCurrentFile( _unzFile );
		ret = unzGoToNextFile( _unzFile );
		[self performSelectorOnMainThread: @selector(updateProgress:) withObject: [NSNumber numberWithInt: ++count] waitUntilDone: YES];
