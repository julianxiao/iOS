...
@interface SingleImageViewController (UtilityMethods)
...
- (void) onResult{
	...
	UIBarButtonItem *layoutButton = [[UIBarButtonItem alloc]
									 initWithTitle:viewTitle style:UIBarButtonItemStylePlain
									 target:self
...
- (void) handleButton
{
	...
	if([item.rightBarButtonItem.title isEqualToString:@"Select"]) 
	{
		item.rightBarButtonItem.title = @"Remove";
		[thumbsDataSource addUnselectedImage:imageNum toPage:-1];
	}
	else	
	{
		item.rightBarButtonItem.title = @"Select";
		[thumbsDataSource movePhotoToUnselected:imageNum];
